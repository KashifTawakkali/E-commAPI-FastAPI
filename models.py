...
class Product(Model):
    id = fields.IntField(pk = True, index = True)
    namer = fields.CharField(max_length=100, null=False, index = True)
    catogry = fields.CharField(max_length=30, index=True)
    ...
    buseness = fields.ForeignKeyField("models.Business",related_name="products")
    
    
user_pydantic = pydantic_model_creator(User, name="User", exclude=("is_verified" , ))
user_pydantic = pydanticIN =pydantic_model_creator(User, name="UserIN",exclude_readonly=True, exclude=("is_verified" , "join_date" ))
user_pydanticOut = pydantic_model_creator(User, name="UserOut",exclude=("password", ))


business_pydentic= pydantic_model_creator(Business, name="Business")
business_pydentic = pydantic_model_creator(Business, name="Business", exclude_readonly=True)


product_pydentic = pydantic_model_creator(Product, name="Product")
product_pydantic = pydantic_model_creator(Product, name="ProductIN", exclude=("percentage_discount", "id"))
